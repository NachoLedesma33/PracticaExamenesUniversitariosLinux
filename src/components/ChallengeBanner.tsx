...
export function ChallengeBanner() {
  ...
  const isScripting = isText && (challenge.category.toLowerCase().includes('scripting') || challenge.category.toLowerCase().includes('finales'));
  ...
  return (
    <div className={`banner-border banner-bg backdrop-blur-sm ${isScripting ? 'flex-1 flex flex-col min-h-0' : ''}`}>
      <div className={`px-4 py-2.5 ${isScripting ? 'flex-1 flex flex-col min-h-0' : ''}`}>
        <div className={`flex items-start gap-3 ${isScripting ? 'flex-1 flex flex-col min-h-0' : ''}`}>
          <div className={`flex-1 min-w-0 ${isScripting ? 'flex flex-col min-h-0 overflow-y-auto' : ''}`}>
            ...
            {!collapsed && isText && !completed && (
              <div className={`${isScripting ? 'flex-1 flex flex-col min-h-0' : ''} mt-3 animate-fade-slide`}>
                <div className={`relative ${isScripting ? 'flex-1 flex flex-col min-h-0' : ''}`}>
                  <textarea
                    ref={inputRef}
                    value={textAnswer}
